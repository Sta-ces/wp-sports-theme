...
class Button_Widget_Register extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$get_linkpage = isset( $instance['linkpage'] ) ? get_page_link($instance['linkpage']) : $this->defaults['linkpage'];
		$get_link     = isset( $instance['link'] ) ? $instance['link'] : $this->defaults['linkpage'];
		...
		if ( empty( $get_text ) ) { return; }

		$output = sprintf( '%s<a href="%s" id="%s" title="%s" target="%s" rel="%s" style="%s" class="%s">%s</a>%s', $args['before_widget'], !empty( $get_link ) && $get_link !== "#" ? esc_url( $get_link ) : esc_url( $get_linkpage ), esc_attr( $get_id ), esc_attr( $get_title ), $get_target ? '_blank' : '_self', $get_target ? 'noopener noreferrer nofollow' : '', implode( '', $get_colors ), esc_attr( $get_class ), esc_html( $get_text ), $args['after_widget'] );
	...
	public function form( $instance ) {
		...
		?><p>
			...
			<?php echo esc_html_x( 'Text:', 'field label', 'button-widget' ); ?>
			</label>
			<input
				type="text"
				class="widefat"
		...
		<p>
			...
			<?php echo esc_html_x( 'Link to:', 'field label', 'button-widget' ); ?>
			</label>
			<input
				type="url"
				class="widefat"
			...
			/>
			<span style="display:block;font-style:italic;margin:.25em 0;">or</span>
			<select id="<?php echo $this->get_field_id('linkpage'); ?>" name="<?php echo $this->get_field_name('linkpage'); ?>" class="widefat" style="width:100%;">
				<?php foreach(get_pages() as $post_type): ?>
					<option <?php selected( $instance['linkpage'], $post_type->ID ); ?> value="<?php echo $post_type->ID; ?>"><?php echo $post_type->post_title; ?></option>
				<?php endforeach; ?>
			</select>
		</p>
		<p>
			...
			<?php echo esc_html_x( 'Open linked address in a new window?', 'field label', 'button-widget' ); ?>
			</label>
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'text_color' ) ); ?>">
			<?php echo esc_html_x( 'Color:', 'field label', 'button-widget' ); ?>
			</label>
			<input
				type="text"
		...
		<p>
			...
			<?php echo esc_html_x( 'Background:', 'field label', 'button-widget' ); ?>
			</label>
			<input
				type="text"
				class="button-widget-color-picker"
	...
	public function update( $new_instance, $old_instance ){
		...
		$instance['link']             = (!empty($instance['link'])) ? esc_url_raw( $new_instance['link'] ) : $instance['link'];
		$instance['linkpage']         = sanitize_text_field( $new_instance['linkpage'] );
		$instance['target']           = self::string_to_bool( $new_instance['target'] );
